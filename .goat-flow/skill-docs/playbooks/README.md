...
| [`release.md`](./release.md) | Cutting a release of the `ardenexal/*` packages: choosing the version, bumping all four lockstep version shapes, proving resolution with the isolation replay, the two-stage human handoff (commit/PR, then `gh release create` on a verified SHA), post-publish verification, and recovery from a bad publish. Project-owned | `gh`, `composer`, `jq` |
