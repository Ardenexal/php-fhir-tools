            ...
            ->children()
                ...
                ->arrayNode('validation')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->arrayNode('message_overrides')
                            ->info('Override violation message templates per constraint. Keys are simple class names (e.g. FHIRFixedValue), values are message templates.')
                            ->useAttributeAsKey('name')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                        ->end()
                        ->scalarNode('terminology_cache_pool')
                            ->defaultNull()
                            ->info('PSR-6 cache pool service ID for terminology validation results. Set to null to disable.')
                        ->end()
                        ->integerNode('terminology_cache_ttl')
                            ->defaultValue(3600)
                            ->min(0)
                            ->info('TTL in seconds for cached terminology results. 0 = no expiry.')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('packages')
