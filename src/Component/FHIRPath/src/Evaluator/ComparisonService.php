...
final class ComparisonService
{
     ...
     * - Equality (`=`): positional comparison — each item at index i in the left
     *   collection must equal the item at index i in the right collection.
     * - Equivalence (`~`): set-based matching (order-independent) with type-normalising
     *   equality (e.g. Integer 1 ~ Decimal 1.0) and duplicate-removal before comparison.
     *
    ...
    private function collectionsEqual(array $left, array $right, bool $useEquivalence): ?bool
    {
        ...
        if (!$useEquivalence) {
            // Equality (=): positional comparison — items must match at the same index
        ...
        // Equivalence (~): set-based matching (order-independent).
