...
| **Extension contexts** (`type=extension`) | ❌ | ❌ | ❌ | Always permitted in v1 |
...
**Extension context type `extension`** is not evaluated. Any extension with a
`type=extension` context is treated as permitted regardless of the declared parent
extension URL.
