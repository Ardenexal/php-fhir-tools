...
abstract class AbstractFHIRNormalizer implements FHIRNormalizerInterface, SerializerAwareInterface
{
    ...
    protected function normalizePrimitiveWithExtensions(mixed $value, ?string $format, array $context, bool $includeExtensions = true): ?array
    {
        ...
        if ($reflection->hasProperty('value')) {
            ...
            if ($valueProperty->isInitialized($value)) {
                ...
                // NOTE: this handles the ->value property of standalone DecimalPrimitive objects.
                // For decimal-typed scalar properties inside resource/complex/backbone normalizers,
                // castNumericScalarForJson() handles the conversion using PropertyMetadata.fhirType.
    ...
    {
        $class = get_class($obj);
        if (array_key_exists($class, $this->decimalPrimitiveCache)) {
            return $this->decimalPrimitiveCache[$class];
        }
        ...
        do {
            ...
            if (!empty($attributes)) {
                return $this->decimalPrimitiveCache[$class] = $attributes[0]->newInstance()->primitiveType === 'decimal';
            }
            $reflection = $reflection->getParentClass();
        } while ($reflection !== false);

        return $this->decimalPrimitiveCache[$class] = false;
    }

    /**
     * Cast a numeric-string scalar to the appropriate PHP numeric type for JSON output.
     *
     * NOTE: this handles inline scalar properties in resource/complex/backbone normalizers.
     * For standalone DecimalPrimitive objects, see normalizePrimitiveWithExtensions().
     *
     ...
     * Returns null when no variant matches (caller should fall back to legacy handling).
     *
